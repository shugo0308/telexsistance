...
# --- ネットワーク・デバイス設定 ---
# 環境変数 'HERMES_IP' があればそれを使い、なければデフォルト(192.168.0.41)を使う
HERMES_IP = os.getenv("HERMES_IP", "192.168.0.41")
DEST_PORT = 5005
...
class PoseEstimator:
    ...
    def __init__(self, width=640, height=480):
        ...
        self.pitch_history = deque(maxlen=self.history_len)
        
        # --- 2. ローパスフィルター用 ---
        # lpf_gainが高いほど「ヌルヌル（重厚）」になり、低いほど「キビキビ（機敏）」になります
        ...
        # PnP用設定
        self.deadzone = 0.2  # 0.2度以下の微細な動きは「静止」とみなして無視する
    ...
    def get_angles_and_distance(self, face_landmarks, manual_offset):
        ...
        # --- 🌟 動的LPFゲインの計算 🌟 ---
        # 動きが激しい(diffが大きい)ほど gain を下げて追従性を上げます
        # 動きが小さいほど gain を上げて(最大0.92)プルプルを抑えます
        dynamic_gain_y = max(0.4, min(0.92, 0.92 - abs(r_yaw) * 0.1))
        dynamic_gain_p = max(0.4, min(0.92, 0.92 - abs(r_pitch) * 0.1))
        
        #---移動平均の計算---
        self.yaw_history.append(r_yaw)
        self.pitch_history.append(r_pitch)
        avg_y = sum(self.yaw_history) / len(self.yaw_history)
        avg_p = sum(self.pitch_history) / len(self.pitch_history)

        #---ローパスフィルター（LPF)---
        # 過去の値と平均値をブレンドして「角」を取る
        new_filtered_y = (self.last_y * dynamic_gain_y) + (avg_y * (1.0 - dynamic_gain_y))
        new_filtered_p = (self.last_p * dynamic_gain_p) + (avg_p * (1.0 - dynamic_gain_p))
                
        # --- 🌟 不感帯(Deadzone)の適用 🌟 ---
        abs_yaw = abs(new_filtered_y)
        abs_pitch = abs(new_filtered_p)
        # 🌟 姿勢に応じた「静止判定用」のしきい値を計算
        # 正面(0度)なら 0.15度、最大角度(38度)なら 0.5度 まで
        # 顔を横に向けるほど、止まろうとする時の「壁」を高くする
        stop_threshold_y = 0.15 + (abs_yaw / 38.0) * 0.5
        stop_threshold_p = 0.15 + (abs_pitch / 14.0) * 0.35
        
        diff_y = abs(new_filtered_y - self.last_y)
        diff_p = abs(new_filtered_p - self.last_p)
        
        if diff_y < stop_threshold_y:
            # 動きが小さければ、強制的に前回の値で固定して微動を消す
            filtered_y = self.last_y 
        else:
            # しきい値を超えた時（明確な移動意思がある時）だけ更新
            filtered_y = new_filtered_y * 0.8 + avg_y * 0.2 
        
        if diff_p < stop_threshold_p:
            filtered_p = self.last_p
        else:
            filtered_p = new_filtered_p * 0.8 + avg_p * 0.2
        
        # 次回のために保存
        self.last_y = filtered_y
        self.last_p = filtered_p
        
        # まだキャリブレーションされていないなら生の角度を返す
        if not self.is_calibrated:
            return filtered_y, filtered_p, r_yaw, r_pitch, dist_mm
        
        # 感度調整用の固定値ロジック (左右対称)
        fixed_denom = 40.0  # この値を大きくすると動きがマイルドに、小さくするとクイックになります
        target_max = 38.0

        raw_diff_y = ((filtered_y - self.offset_yaw + manual_offset + 180) % 360 - 180)
        
        # 単純な割り算でマッピング（左右の区別をなくす）
        #y_out = (self.filtered_yaw / fixed_denom) * target_max
        y_out = (raw_diff_y / fixed_denom) * target_max
        # 最終リミッター
        y_out = max(-38.0, min(38.0, y_out))
        # 0.2度刻みに丸める
        y_out = round(y_out * 5) / 5
        
        # get_angles_and_distance 内の Pitch 計算部分
        # 生の pitch (r_pitch) から、中心設定時の offset_pitch を引く
        #p_out = r_pitch - self.offset_pitch
        p_out = filtered_p - self.offset_pitch
        # 戻り値も補正後の p_out を使うように変更
        return y_out, p_out, filtered_y, filtered_p, dist_mm
    ...
    try:
        ...
        with mp_face_mesh.FaceMesh(
            ...
            while True:
                ...
                chunk = await asyncio.to_thread(proc.stdout.read, 4096)#65536)
                if not chunk:
                    break
                #カメラデータをbufferに継ぎ足し
                buffer += chunk
                ...
                if results.multi_face_landmarks:
                    ...
                    if current_req:
                        ...
                        # ブラウザからゼロ点補正が押された時の動作
                        if current_req == "center":
                            estimator.set_center(r_y, r_p)
                        # ブラウザからオフセット初期化が押された時の動作
                        elif current_req == "reset":
                            estimator.is_calibrated = False
                            estimator.last_y = 0.0
                            estimator.last_p = 0.0
                            estimator.yaw_history.clear()
                            estimator.pitch_history.clear()
                            print("🔄 Reset: オフセットとキャリブレーションを初期化しました")
                    ...
                    try:
                        udp_sock.sendto(message.encode(), (HERMES_IP, DEST_PORT))
                    except Exception as e:
                        print(f"UDP Send Error: {e}")
                else:
                    # --- 2. 💡 顔が見つからなかった時の処理 (else) ---
                    # LEDを暗めの待機色にする
                    pixels.brightness = 0.05
                    pixels.fill((150, 100, 50)) # 落ち着いたアンバー色
                
                    # ※ここではUDP送信を「しない」ことで、エルメス側に
                    # 「最後の場所で止まって待つ」という挙動をさせることができます。
