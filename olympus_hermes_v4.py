...
class SmoothTracker:
    def __init__(self):
        ...
        self.speed_gain = 0.65       #目標値に移動するスピードのゲイン
        self.base_accel = 0.5        #加速度の限界(目標値が近い時)
        self.boost_accel = 1.5       #加速度の限界(目標値が遠い時)
        self.damping = 0.94          #動いた後のブレーキの強さ
...
async def servo_drive_loop():
    print("🚀 servo_drive_loop STARTED!", flush=True)
    
    try:
        while True:
            # 💡 kit が None、または tracker が未初期化なら処理をスキップ
            if kit is None or tracker is None:
                # ログが流れすぎないよう、1秒待機してループを継続
                # print("⚠️ ServoKit not available. Skipping step...")
...
class DriveState:
    def __init__(self):
        self.last_thr = 0.0  # 追加：最新のアクセル量
        self.last_brk = 0.0  # 追加：最新のブレーキ量
        self.is_back_requested = False  # クライアントがバックを求めているか
        self.is_reversing_seq = False   # ダブルタップの実行中か
        self.reverse_task = None

drive_state = DriveState()

# --- 1. サーボ更新用のバックグラウンドタスク ---
async def servo_update_loop():
    global current_throttle_val
    while True:
        # --- A. バックモードの処理 ---
        if drive_state.is_back_requested:
            # バックに入った瞬間、前進の値をリセット
            current_throttle_val = ANGLE_STOP
            
            # ダブルタップシーケンスが動いていないなら開始
            if not drive_state.is_reversing_seq:
                drive_state.is_reversing_seq = True
                async def reverse_sequence():
                    try:
                        # 1回目：バックに入れてブレーキを解除させる
                        ...
                        # 2回目：ここで実際にバックが始まる
                        kit.servo[ESC_CH].angle = ANGLE_MAX_REV
                        # シーケンス終了後も、この角度を維持し続けるためのループ
                        while drive_state.is_back_requested:
                            kit.servo[ESC_CH].angle = ANGLE_MAX_REV
                            await asyncio.sleep(0.05)
                    finally:
                        drive_state.is_reversing_seq = False
                        
                drive_state.reverse_task = asyncio.create_task(reverse_sequence())
                
        # --- B. 前進・エンブレ・ブレーキの処理 ---
        else:
            # バックモードでないなら、タスクがあればキャンセル
            if drive_state.reverse_task:
                drive_state.reverse_task.cancel()
                drive_state.reverse_task = None
                drive_state.is_reversing_seq = False
            
            thr = drive_state.last_thr
            brk = drive_state.last_brk
            
            #アクセルオンの時の処理
            if thr > 0:
                rise = thr * THROTTLE_GAIN
                current_throttle_val += (ANGLE_MAX_FWD - current_throttle_val) * rise 
            #ブレーキ時の処理(アクセル、ブレーキ同時踏みの場合ブレーキを優先させる)
            elif brk > 0:
                decay = brk * BRAKE_GAIN
                current_throttle_val -=  (current_throttle_val - ANGLE_STOP)*decay
            else:
                # 前進中または停止中の時
                diff = current_throttle_val - ANGLE_STOP
                # エンブレ計算（diffが0なら結果的に何もしない）
                if abs(diff) < 0.1:
                    current_throttle_val = ANGLE_STOP
                else:
                    # 💡 ここで常に減衰させ続ける
                    current_throttle_val -= diff * ENGINE_BRAKE_GAIN
            
            # 書き込み（前進・停止時のみここを通る）
            current_throttle_val = max(ANGLE_STOP, min(ANGLE_MAX_FWD, current_throttle_val))
            if kit is not None:
                kit.servo[ESC_CH].angle = current_throttle_val
...
async def lifespan(app: FastAPI):
    ...
    task3 = asyncio.create_task(servo_update_loop())
...
async def drive_endpoint(websocket: WebSocket):
    ...
    # 💡 接続時に警告を出す
    if kit is None:
        print("❌ ServoKit is None. Closing connection.")
        # ステータスコード 1011 (Internal Error) で切断
        await websocket.close(code=1011, reason="ServoKit not initialized")
        return # 関数を終了させる
    ...
    try:
        while True:
            ...
            try:
                ...
                drive_state.last_thr = data.get('throttle', 0)
                drive_state.last_brk = data.get('brake', 0)
                drive_state.is_back_requested = data.get('isBackMode', False)
...
#---セマフォの初期化---
video_lock = asyncio.Semaphore(1)
...
async def video_endpoint(websocket: WebSocket):
    ...
    if video_lock.locked():
        await websocket.close(code=4001, reason = "Busy")
        print("⚠️ 接続拒否: すでに別のクライアントが接続しています。")
        return
        ...
        try:
            while True:
                ...
                if frame is not None:
                    success, buf = await asyncio.to_thread(
